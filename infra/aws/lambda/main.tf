locals {
  serivces_conf_file = [for fn in fileset("../../services", "*/config.yaml") : fn]
...
resource "aws_lambda_function" "this" {
  ...
  image_uri     = "778424175012.dkr.ecr.us-east-1.amazonaws.com/orders-management-ecr:${each.name}-${each.version}"
  package_type  = "Image"
